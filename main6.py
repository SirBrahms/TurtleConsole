...
def posJmp():
    global Ypos, ctrlList, editormode, feditormode, KEY_RETURN, suppress_input #initialisierung wichtiger Variabeln (r/w zugriff)
...
def construct_arg(ctrlList):
    ...
    global saveVar #Zugriff auf den Default-Speicherort
    ...
    if(strLs[0].upper() == "TSTART" and len(strLs) == 1 and not secTurtleMode):
        ...
        t1.clearStates()
    elif(strLs[0].upper() == "TSAVE" and len(strLs) >= 2 and secTurtleMode):
        if(t1.savePlayground(saveVar + strLs[1] + ".png", "png")):
            strLs.pop(0)
            savestring = StringList(strLs)
            msgDlg("Datei erfolgreich Gespeichert!\nSpeicherort: " + saveVar + savestring + ".png", title = "Erfolg")
        else:
...
def interpret(ctrlList):
    ...
    global KEY_RETURN #initialisierung wichtiger Buchstaben (r/w zugriff)
    ...
    if(flag == "SC"):
        ...
        if((savePlayground(saveVar + ".png", "png"))): #überprüft, ob die datei erfolgreich gespeichert werden konnte
            try:
    ...
    if(ctrlList == ['E', 'C', 'H', 'O'] and flag == "DEBUG"): #überprüfung der Liste zur interpretation
        ...
        draw_letter("blob")
        flag = "SETVOL"
    elif(StringList(ctrlList) == "SVEM" and flag == "FEDIT"): #command zur Speicherung eines FEDIT-"Dokumentes"
        draw_letter("blob")
        flag = "SVEM"
    #elif(...)
    else:
...
def saveEditorfile(name):
    #Funktion um die in FEDIT hergestellte Datei zu Speichern
    global ctrlList #initialisierung der CtrlList zur speicherung derselben in einer Datei
    global saveVar #initialisierung des default-Speicherortes
    ...
    try:
        ...
        msgDlg("Datei Erfolgreich gespeichert\nSpeicherort: "+saveVar + StringList(name))
    except:
        bkup_list = []
        return
...
############################################
#Funktion zur Zeichnung der Buchstaben
############################################
