...
try:
    ...
    print(error)
enterCount = ctrlList.count("enter")
...
def deleteChar():
    global ctrlList, KEY_RETURN, Ypos, editormode
    count = 0
    if(ctrlList != []):   
        ctrlList.pop()
        clear("black")
...
def draw_letter(key, add = True):
    global ctrlList, col, editormode, suppress_input #initialisierung der Command-Liste (r/w zugriff)
    """
    ...
    *Bei punkt, enter, komma, blob sind diese Abgleiche nicht vonnöten, da die Funktion selbst sie bereits übernimmt
    ...
    if(suppress_input and key != KEY_RETURN):
        return
